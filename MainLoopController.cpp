...
MainLoopController::MainLoopController(const std::string &kafkaBroker, const std::string &kafkaTopic,
                                       uint32_t kafkaPartition, const std::string &filesDirectory,
...
void MainLoopController::Run()
{
    ...
    while(!IsShutdownFlagSet()) {
        try {
                ...
                if (filesystem::is_regular_file(dirIterator->status()) &&
                    ...
                    if (parser.IsReady()) {
                        ...
                        if (lastPostponeReason != parser.GetPostponeReason()) {
                            ...
                            logWriter.Write("CDR processing postponed due to: " + lastPostponeReason, LogWriter::mainThr);
                        }
                        Sleep();
                    }
            ...
            logWriter.Write("ERROR in MainLoopProcessor::Run:", LogWriter::mainThr, error);
